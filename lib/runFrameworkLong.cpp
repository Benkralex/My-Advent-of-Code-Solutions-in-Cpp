...
class RunFrameworkLong {
private:
    vector<string> examplePaths;
    vector<long> exampleOutputs;
    string inputPath;
    std::function<long(string, bool)> func;
    bool debug;
public:
    RunFrameworkLong(
        const vector<string> examplePaths_, 
        const vector<long> exampleOutputs_, 
        const string inputPath_, 
        std::function<long(string, bool)> func_,
        bool debug_ = false
    )
    ...
    bool runExample(const int index) {
        ...
        long start = now();
        long result = func(examplePaths[index], debug);
        long end = now();
    ...
    void runInput() {
        ...
        long start = now();
        long result = func(inputPath, debug);
        long end = now();
